...
func startLocalServer() string {
	...
	mux.HandleFunc("/todos/1", func(w http.ResponseWriter, r *http.Request) {
		...
		json.NewEncoder(w).Encode(JSONPlaceholderTodo{
			UserID:    1,
			ID:        1,
			Title:     "测试待办事项",
			Completed: false,
...
func main() {
	...
	// 示例4: 自定义请求头
	fmt.Println("\n=== 示例4: 带自定义请求头的请求 ===")
	resp, err = client.Get(ctx, "/headers", &httpclient.RequestOptions{
	...
	if err != nil {
		log.Fatalf("带自定义请求头的请求失败: %v", err)
	}
	fmt.Printf("状态码: %d\n", resp.StatusCode)

	// 解析响应体，显示请求头
	var headersResp map[string]string
	if err := json.Unmarshal(resp.Body, &headersResp); err == nil {
	...
	// 示例5: 禁用重试的请求
	fmt.Println("\n=== 示例5: 禁用重试的请求 ===")
	resp, err = client.Get(ctx, "/todos/1", &httpclient.RequestOptions{
		Retry: false,
	})
	if err != nil {
		log.Fatalf("禁用重试的请求失败: %v", err)
	}
	fmt.Printf("状态码: %d\n", resp.StatusCode)
